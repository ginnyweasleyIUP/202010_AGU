...
#year_start and year_stop in years BP
...
load_sisal_data <- function(prefix = "", year_start, year_stop, min_period, min_dating, min_d18O){
  ...
  #     at least X dates within the past millenium (between year_start and year_stop)
  #     at least X measurements within the past millenium (between year_start and year_stop)
  #     at least X 600y coverage
  ...
  sample_past1000 <- sample_tb_new_filtered %>% filter(entity_id %in% entities_minperiod$entity_id) %>% 
    distinct(entity_id, interp_age, d18O_measurement, d13C_measurement, mineralogy) %>% filter(interp_age<(year_start) & interp_age > (year_stop))
  ...
  dating_all_past1000 <- sample_tb_new_filtered %>% filter(entity_id %in% entities_minperiod$entity_id) %>% 
    ...
    filter(interp_age<(year_start) & interp_age > (year_stop))
...
return(list(site_past1000, sample_past1000, dating_all_past1000))
  ...
  #      entities_dating, path, prefix, entities_d18O, entities_minperiod, dating_tb_withouthiatus, site_past1000, sample_past1000, dating_all_past1000)
